...
RWSelectSwitchEnum rw_select;

RecvDataStruct  recv_data;
...
static commx_cb read_argu_cb = NULL;
static commx_cb write_argu_cb = NULL;
static commx_cb read_eeprom_cb = NULL;
static commx_cb write_eeprom_cb = NULL;

//-----------------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------------
static void r_argu_ctrl (void)
{
	rw_step = RWEnd;//置状态
	rw_select = RWIsEnd;
	read_argu_cb (rw_data);//执行用户的回调函数
	read_argu_cb = NULL;
	rw_status = KReadWriteIdle;//状态置为空闲
}
//-----------------------------------------------------------------------------------------
static void w_argu_ctrl (void)
{
	rw_step = RWEnd;
	rw_select = RWIsEnd;
	write_argu_cb (rw_data);
}
//-----------------------------------------------------------------------------------------
//写0x97控制
...
static void r_data_ctrl (int data)
{
	...
	case IsReadEeprom:
		rw_step = RWEnd;
		rw_select = RWIsEnd;
		read_eeprom_cb (data);//执行用户的回调函数，操作完成
		break;
	default:
		rw_status = KReadWriteErr;//其他状态，没商量，肯定是哪里错误了，直接置为错误
		break;
...
void read_argument (const int axis_id, const int addr, commx_cb cb)
{
	...
	if (rw_status == KReadWriteIdle)
	{
		...
		{
			recv_data.id = axis_id;
			recv_data.addr = addr;
			recv_data.cb= cb;

			rw_select = RArguStart;
		}
	}
	else
	{
...
//-----------------------------------------------------------------------------------------
static void read_argu (const int axis_id, const int addr, commx_cb cb)
{
	if (rw_status == KReadWriteIdle)
	{
		if (axis_id < 0 || axis_id >= 4 || addr < 0 || addr > 0xb6 || NULL == cb)
...
//			rw_axis_id = recv_axis_id;//轴号
			rw_addr = 0x97;//地址
//			rw_data = recv_axis_id | (0x2000 + (recv_addr << 2));//控制信息，读位置1 + 10位地址
...
//				rw_axis_id = recv_axis_id;//轴号
//				rw_addr = 0x97;//地址
//				rw_data = recv_axis_id | (0x4000 + (recv_addr << 2));//控制信息，读位置1 + 10位地址
...
//-----------------------------------------------------------------------------------------
static void cb_test (int length)
{}
//-----------------------------------------------------------------------------------------
static void rw_select_ctrl (void)
{
	switch (RArguStart)
	{
	case RWNotStart:
		break;
	case RArguStart:
//		read_argu (recv_data.id, recv_data.addr, recv_data.cb);//读参操作
		read_argu (0, 0x24, cb_test);
		break;
	case WArguStart:
//		comm_write_argument ();//开始写参数操作
		break;
...
//    recv_data = 0;//数据
    ...
    rw_step = NotStart;
    is_rw_eeprom = IsNotStart;
    rw_select = RWNotStart;
    array_init ();
}
//-----------------------------------------------------------------------------------------
static void rw_status_ctrl (void)
{
	switch (rw_status)
	{
